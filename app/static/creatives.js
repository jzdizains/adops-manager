...
(function () {
  ...
  function openDrawer(cid) {
    ...
    UI.get("/creatives/" + cid + "/detail").then(function (d) {
      ...
      if (d.kind === "carousel") {
        var ct = $("#dwCaption"), ctimer = null, clast = d.ad_text || "";
        ct.addEventListener("input", function () { clearTimeout(ctimer); $("#dwCaptionSt").textContent = "…"; ctimer = setTimeout(function () { if (ct.value === clast) { $("#dwCaptionSt").textContent = "saved"; return; } UI.post("/creatives/" + d.id + "/caption", { text: ct.value }).then(function (r) { clast = r.ok ? r.text : clast; $("#dwCaptionSt").textContent = r.ok ? "saved" : "couldn't save"; }); }, 700); });
      }
