...
(function () {
  ...
  function openDrawer(cid) {
    ...
    UI.get("/creatives/" + cid + "/detail").then(function (d) {
      ...
      var media = d.kind === "video" && d.file ? '<video src="' + esc(d.file) + '" controls playsinline preload="metadata" poster="' + esc(d.poster) + '"></video>' : '<img src="' + esc(d.poster) + '" alt="">';
