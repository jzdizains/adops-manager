...
def callback(request: Request, db: Session = Depends(get_db)):
    ...
    return render(request, "oauth_result.html", {"ok": True, "detail":
                  ...
                  + (f" under BC {result['bc_id']}" if result.get("bc_id") else "") + "."})
...
def sync_accounts(db: Session, access_token: str, refresh_token: str = "",
    ...
    try:
        ...
        if bcs:
            bc_id = str(bcs[0].get("bc_info", {}).get("bc_id") or bcs[0].get("bc_id", ""))
            page = 1
            while True:
    ...
    for adv in advertisers:
        ...
        row.owner_bc_id = bc_id or row.owner_bc_id
    ...
    return {"count": len(advertisers), "bc_id": bc_id}
