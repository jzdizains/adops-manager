...
async def click_register(request: Request, db: Session = Depends(get_db)):
    ...
    row = tracking.record_click(db, source=source, ttclid=ttclid, tt_campaign_id=str(d.get("tt_cid") or ""),
                                ...
                                url=str(d.get("url") or ""), referrer=str(d.get("ref") or ""), how="direct", ttp=str(d.get("ttp") or ""), vid=str(d.get("vid") or ""))
    db.commit()
...
async def lander_beacon(request: Request, db: Session = Depends(get_db)):
    ...
    if stored:
        ...
        if d.get("step") == "view":
            ...
            lpv_events.enqueue(d, _ip(request), request.headers.get("user-agent", ""))   # landing-view conversion, when set (background)
    return Response(status_code=204, headers=CORS)
