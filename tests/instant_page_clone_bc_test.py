...
check("job body: per-account copy through the page editor API, verified by re-read, pause, stop check", "r = clone_one(db, page_id, name, acct, new_url=new_url, new_text=new_text)" in r and "if should_stop and should_stop():" in r and "_time.sleep(1.5)" in r and "sync_account(db, acct)" in r.split("def clone_one")[1])
