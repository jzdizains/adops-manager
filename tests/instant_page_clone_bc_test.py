...
check("job body: per-account try, re-read, pause, stop check", "spark_web_api.clone_instant_page(page_id, from_advertiser_id, adv)" in r and "if should_stop and should_stop():" in r and "_time.sleep(1.0)" in r and "sync_account(db, acct)" in r.split("def clone_to_many")[1])
