...
check("routes carry the optional re-point; the link must be http(s)", 'new_url: str = Form("")' in ip and ip.count("The new button link must start with http:// or https://") == 3 and '"new_url": new_url' in ip)  # clone, clone-bc, clone-multi
...
check("UI: clone button per page name (published copy only) opens the account pop-up with the optional link re-point", 'class="btn sm ip-clone" {{ \'disabled\' if not web_ready or not g.source }}' in th and "UI.pickAccounts({" in js and 'name: "new_url"' in js and "(r.values && r.values.new_url)" in js)
