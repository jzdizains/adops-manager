...
check("single clone route + BC route, both need the web cookies and the view", r.count("if not spark_web_api.load_cookies():") == 2 and '@router.post("/lead-forms/clone")' in r and '@router.post("/lead-forms/clone-bc")' in r and r.count("sc.allows(") >= 4)
...
check("runs as a slow-lane job with progress + stop; one failure never stops the rest", '@jobs.handler("lead_form_clone_all")' in h and '"lead_form_clone_all"' in j.split("SLOW_KINDS = {")[1][:120] and "if should_stop and should_stop():" in r.split("def clone_to_many")[1] and "_time.sleep(1.0)" in r)
