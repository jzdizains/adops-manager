...
check("single clone route + BC route, both need the web cookies and the view", r.count("if not spark_web_api.load_cookies():") == 3 and '@router.post("/lead-forms/clone")' in r and '@router.post("/lead-forms/clone-bc")' in r and r.count("sc.allows(") >= 4)  # +1: read-only /lead-forms/inspect also guards cookies
